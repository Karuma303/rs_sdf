...
mod tests {
    ...
    use rs_sdf::data::input::{InputBoolBuffer, InputField};

    // helper method to get an empty source field
    fn get_source_0_0() -> InputField {
        InputField::from(InputBoolBuffer::new(&[], 0, 0))
    }
    ...
    fn distance_field_has_correct_dimension() {
        let source = InputField::from(InputBoolBuffer::new(&[true, true, true], 3, 1));
        let df = DistanceField::new(&source);

        assert_eq!(df.width, 3);
        assert_eq!(df.height, 1);

        let source = InputField::from(InputBoolBuffer::new(&[true, true, true], 1, 3));
        let df = DistanceField::new(&source);
